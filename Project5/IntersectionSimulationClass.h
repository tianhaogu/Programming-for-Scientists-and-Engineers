...
class IntersectionSimulationClass {
  ...
  public:
    ...
    //state.
    void readParametersFromFile(const std::string &paramFname); //Name of text file to read params from
    ...
    void handleArrival(EventClass &thisEvent);
    
    //handel the real event of light change
    void handleLightChange(EventClass &thisEvent);

    //handle the first event that exceeds the time limit
    void handleExceededEvent(EventClass &thisEvent);
